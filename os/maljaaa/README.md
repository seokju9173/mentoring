...
<picture>
  <img src="https://blog.kakaocdn.net/dn/bQcGID/btqEyJSBKLx/XKriHXUNpVstVbslZy54g1/img.png">
</picture>

```
메모리가 할당되고 해제되는 작업이 반복될 때 작은 메모리 중간중간에 사용하지 않는 메모리가 많이 존재해서
총 메모리 공간은 충분하지만 실제로 할당할 수 없는 문제를 External fragmentation, 외부단편화라고 한다.
```

> 이를 해결하기 위해 비어있는 공간을 연속적인 공간으로 만들고 움직이는 작업을 Compaction 이라고 함 -> Paging의 배경

> Internal fragmentation

<picture>
  <img src="https://blog.kakaocdn.net/dn/bjRir2/btqExZvakQc/aMf6hXyKAHgfYbVpJWC4Zk/img.png">
</picture>

```
프로세스 A가 페이지 한 블록의 크기보다 작은 추가적인 크기를 가지고 있어서 공간이 남는 경우
```

### 2. Paging(페이징)
 
```
Logical address와 Physical memory를 각각 동일한 크기로 자르고 배치하여 자투리 공간이 발생하지 않도록 하는 기법
```
...
* Internal fragmentation이 생길 수 있으나 매우 작으므로 메모리를 거의 100프로 가깝게 다 쓸 수 있는 방법이다.

#### Page vs Frame

* Page : Logical address space를 동일하게 크기로 나눈것 
