...
```
프로세스의 코드에 정의돈 절차에 따라 CPU에 작업 요청을 하는 실행 단위
```
...
|멀티프로세싱|<ul><li>CPU를 여러 개 사용하여 여러 개의 스레드를 동시에 처리하는 작업 환경</li></ul><ul><li>하나의 컴퓨터에 여러 개의 CPU, 하나의 CPU 내 여러 개의 코어에 스레드 배정, 네트워크로 연결된 여러 컴퓨터에 스레드를 나누어 협업하는 분산시스템 => 멀티프로세싱</li></ul>|
